...
mainLink = "https://nutrition.sa.ucsc.edu/longmenu.aspx?sName=UC+Santa+Cruz+Dining&locationNum=40&locationName=John+R.+Lewis+%26+College+Nine+Dining+Hall&naFlag=1&WeeksMenus=UCSC+-+This+Week%27s+Menus&dtdate="
#change date.month etc to specific day if you don't want todays menu
monthString = str(date.month) + "%2f"
dayString = str(date.day) + "%2f"
yearString = str(2024) + "&mealName="
# Meal Names:
# - Breakfast
# - Lunch
# - Dinner
...
def getMainLink():
    return mainLink + monthString + dayString + yearString + meal


def setResponse(link, header):
    response = requests.get(link)
    
    
def getCleanHeader(inputStr):
    headerList = [[item for item in rowString.split(": ")] for rowString in inputStr.split("\n")]
    headers = {x[0]:x[1] for x in headerList}
    return headers
    
    
def findMacronutrient(response, nutrientName):
...
def getAllMacros(response):
    macroList = ["Total Fat", "Tot. Carb.", "Protein"]
    macros = {}
    macros["Calories"] = float(getCalories(response))
    for macro in macroList:
        macros[macro] = float(findMacronutrient(response,macro))
    return macros
def addPrefix(string):
    return linkPrefix + string 

headers = getCleanHeader(headerString)
responseMain = requests.get(getMainLink(), headers=headers, verify=False)
